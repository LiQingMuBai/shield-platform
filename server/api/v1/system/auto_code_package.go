...
func (a *AutoCodePackageApi) Delete2(c *gin.Context) {
	var info common.GetById
	_ = c.ShouldBindJSON(&info)
	err := tgUserService.ResetTimes(c.Request.Context(), info.ID)
	if err != nil {
		global.GVA_LOG.Error("删除失败!", zap.Error(err))
		response.FailWithMessage("删除失败", c)
		return
...
func (a *AutoCodePackageApi) All2(c *gin.Context) {
	data, err := tgUserService.GetAllUserInfoList(c.Request.Context())
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败", c)
		return
