...
<script setup>
import {
  createSysAnnouncementsInfo,
  deleteSysAnnouncementsInfo,
  deleteSysAnnouncementsInfoByIds,
  updateSysAnnouncementsInfo,
  findSysAnnouncementsInfo,
  getSysAnnouncementsInfoList
} from '@/api/ushield/sysAnnouncementsInfo'
...
const getTableData = async() => {
  const table = await getSysAnnouncementsInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要发送吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteSysAnnouncementsInfoFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteSysAnnouncementsInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSysAnnouncementsInfoFunc = async(row) => {
    const res = await findSysAnnouncementsInfo({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSysAnnouncementsInfoFunc = async (row) => {
    const res = await deleteSysAnnouncementsInfo({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '派发成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createSysAnnouncementsInfo(formData.value)
                  break
                case 'update':
                  res = await updateSysAnnouncementsInfo(formData.value)
                  break
                default:
                  res = await createSysAnnouncementsInfo(formData.value)
                  break
