  ...
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserAddressMonitorEventFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.deletedAt }}
                    ...
                    <el-descriptions-item label="days字段">
    {{ detailForm.days }}
</el-descriptions-item>
...
import {
  ...
  deleteUserAddressMonitorEvent,
  deleteUserAddressMonitorEventByIds,
  updateUserAddressMonitorEvent,
  findUserAddressMonitorEvent,
  getUserAddressMonitorEventList
...
const getTableData = async() => {
  const table = await getUserAddressMonitorEventList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserAddressMonitorEventFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserAddressMonitorEventByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserAddressMonitorEventFunc = async(row) => {
    const res = await findUserAddressMonitorEvent({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserAddressMonitorEventFunc = async (row) => {
    const res = await deleteUserAddressMonitorEvent({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
