  ...
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserTrxSubscriptionsFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteUserTrxSubscriptions,
  deleteUserTrxSubscriptionsByIds,
  updateUserTrxSubscriptions,
  findUserTrxSubscriptions,
  getUserTrxSubscriptionsList
...
const getTableData = async() => {
  const table = await getUserTrxSubscriptionsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserTrxSubscriptionsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserTrxSubscriptionsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserTrxSubscriptionsFunc = async(row) => {
    const res = await findUserTrxSubscriptions({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserTrxSubscriptionsFunc = async (row) => {
    const res = await deleteUserTrxSubscriptions({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
