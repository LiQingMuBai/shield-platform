  ...
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.updatedAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserEnergyOrdersFunc(scope.row)">编辑</el-button>
...
<script setup>
import {
  createUserEnergyOrders,
  deleteUserEnergyOrders,
  deleteUserEnergyOrdersByIds,
  updateUserEnergyOrders,
  findUserEnergyOrders,
  getUserEnergyOrdersList
} from '@/api/ushield/userEnergyOrders'
...
const getTableData = async() => {
  const table = await getUserEnergyOrdersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserEnergyOrdersFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserEnergyOrdersByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserEnergyOrdersFunc = async(row) => {
    const res = await findUserEnergyOrders({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserEnergyOrdersFunc = async (row) => {
    const res = await deleteUserEnergyOrders({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserEnergyOrders(formData.value)
                  break
                case 'update':
                  res = await updateUserEnergyOrders(formData.value)
                  break
                default:
                  res = await createUserEnergyOrders(formData.value)
                  break
