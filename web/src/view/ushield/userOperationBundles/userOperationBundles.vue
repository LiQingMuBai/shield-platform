  ...
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserOperationBundlesFunc(scope.row)">编辑</el-button>
...
<script setup>
import {
  createUserOperationBundles,
  deleteUserOperationBundles,
  deleteUserOperationBundlesByIds,
  updateUserOperationBundles,
  findUserOperationBundles,
  getUserOperationBundlesList
} from '@/api/ushield/userOperationBundles'
...
const getTableData = async() => {
  const table = await getUserOperationBundlesList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserOperationBundlesFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserOperationBundlesByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserOperationBundlesFunc = async(row) => {
    const res = await findUserOperationBundles({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserOperationBundlesFunc = async (row) => {
    const res = await deleteUserOperationBundles({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserOperationBundles(formData.value)
                  break
                case 'update':
                  res = await updateUserOperationBundles(formData.value)
                  break
                default:
                  res = await createUserOperationBundles(formData.value)
                  break
