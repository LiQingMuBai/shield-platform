...
<!--            <el-table-column align="left" label="网络" prop="network" width="120" />-->
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserAddressMonitorFunc(scope.row)">编辑</el-button>
...
<script setup>
import {
  createUserAddressMonitor,
  deleteUserAddressMonitor,
  deleteUserAddressMonitorByIds,
  updateUserAddressMonitor,
  findUserAddressMonitor,
  getUserAddressMonitorList
} from '@/api/ushield/userAddressMonitor'
...
const getTableData = async() => {
  const table = await getUserAddressMonitorList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserAddressMonitorFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserAddressMonitorByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserAddressMonitorFunc = async(row) => {
    const res = await findUserAddressMonitor({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserAddressMonitorFunc = async (row) => {
    const res = await deleteUserAddressMonitor({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     ...
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return btnLoading.value = false
              ...
              switch (type.value) {
                case 'create':
                  res = await createUserAddressMonitor(formData.value)
                  break
                case 'update':
                  res = await updateUserAddressMonitor(formData.value)
                  break
                default:
                  res = await createUserAddressMonitor(formData.value)
                  break
