  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="订单" prop="orderNo" width="120" />
          ...
          <el-table-column align="left" label="交易" prop="txHash" width="120" />

          <el-table-column align="left" label="金额" prop="amount" width="120" />

          <el-table-column align="left" label="区块" prop="block" width="120" />


          <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            <template #default="scope">
            <el-button  type="primary" link class="table-button" @click="getDetails(scope.row)"><el-icon style="margin-right: 5px"><InfoFilled /></el-icon>查看</el-button>
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserUsdtDepositsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.address }}
                    ...
                    <el-descriptions-item label="tx_hash">
    {{ detailForm.txHash }}
</el-descriptions-item>
...
import {
  ...
  deleteUserUsdtDeposits,
  deleteUserUsdtDepositsByIds,
  updateUserUsdtDeposits,
  findUserUsdtDeposits,
  getUserUsdtDepositsList
} from '@/api/ushield/userUsdtDeposits'
...
const getTableData = async() => {
  const table = await getUserUsdtDepositsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteUserUsdtDepositsFunc(row)
        })
    }
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserUsdtDepositsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserUsdtDepositsFunc = async(row) => {
    const res = await findUserUsdtDeposits({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserUsdtDepositsFunc = async (row) => {
    const res = await deleteUserUsdtDeposits({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
