  ...
  <div>
   ...
   <template #default="scope">{{ formatDate(scope.row.createdAt) }}</template>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateUserTrxDepositsFunc(scope.row)">编辑</el-button>
...
import {
  ...
  deleteUserTrxDeposits,
  deleteUserTrxDepositsByIds,
  updateUserTrxDeposits,
  findUserTrxDeposits,
  getUserTrxDepositsList
...
const getTableData = async() => {
  const table = await getUserTrxDepositsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteUserTrxDepositsByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateUserTrxDepositsFunc = async(row) => {
    const res = await findUserTrxDeposits({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteUserTrxDepositsFunc = async (row) => {
    const res = await deleteUserTrxDeposits({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
